...
def _build_user_dashboard(user: dict, bot_user_name: str) -> tuple[str, ReplyKeyboardMarkup]:
...
"
    ...
    kb = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🏆 Reytinglar"), KeyboardButton(text="⚙️ Boshqalar")]
        ],
...
async def cb_user_menu(message: Message, bot: Bot, state: FSMContext) -> None:
    """Asosiy menyuga qaytish."""
    await state.clear()
    if not _bot_username:
        await set_bot_username(bot)

    user_dict = await get_user_profile(message.from_user.id)
    if not user_dict:
    ...
    if query.message:
        ...
        await message.answer("G'ildirakni bosib o'zingizni sinab ko'ring!", reply_markup=reply_kb)
        except Exception:
            pass
    


@router.message(F.text == "🚀 Do'stlarga Ulashish")
async def cb_user_share(message: Message, bot: Bot) -> None:
...
async def cb_user_profile(message: Message, bot: Bot) -> None:
...
async def cb_user_spin_wheel(query: CallbackQuery, bot: Bot) -> None:
    ...
    if query.message:
        try:
            await message.answer("🎡 <i>G'ildirak aylanmoqda... ⏳</i>", parse_mode="HTML")
...
async def cb_user_weekly_top(message: Message) -> None:
    ...
    if True:
        await message.answer("".join(lines), parse_mode="HTML", reply_markup=kb)
...
async def cb_user_top(message: Message) -> None:
    ...
    if query.message:
        ...
        await message.answer("👆 Barcha aktiv konkurslar ro'yxati.", reply_markup=get_contests_keyboard())
...
async def cb_admin_promo_post(query: CallbackQuery, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        ...
        if query.message:
            await message.answer(f"✅ Post muvaffaqiyatli kanalingizga e'lon qilindi va jonli rejimga o'tkazildi!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Bosh menyu", callback_data="admin:menu")]]))
...
async def handle_join_request(update: ChatJoinRequest, bot: Bot) -> None:
    ...
    kb = get_contests_keyboard()
    # We need to keep inline buttons for actual contest joins!
